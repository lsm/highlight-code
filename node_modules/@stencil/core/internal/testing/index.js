/*
 Stencil Testing Platform v1.12.0 | MIT Licensed | https://stenciljs.com
 */
'use strict';
...
const attachStyles = (hostRef) => {
    ...
    const scopeId = addStyle(appData.BUILD.shadowDom && exports.supportsShadow && elm.shadowRoot ? elm.shadowRoot : elm.getRootNode(), cmpMeta, hostRef.$modeName$, elm);
...
const setAccessor = (elm, memberName, oldValue, newValue, isSvg, flags) => {
    if (oldValue !== newValue) {
        ...
        else if (appData.BUILD.vdomListener && (appData.BUILD.lazyLoad ? !isProp : !elm.__lookupSetter__(memberName)) && memberName[0] === 'o' && memberName[1] === 'n') {
            // Event Handlers
        ...
        if (checkSlotRelocate) {
            ...
            for (; i < relocateNodes.length; i++) {
                ...
                if (!nodeToRelocate['s-ol']) {
                    ...
                    orgLocationNode = appData.BUILD.isDebug || appData.BUILD.hydrateServerSide ? originalLocationDebugNode(nodeToRelocate) : doc.createTextNode('');
...
const scheduleUpdate = (hostRef, isInitialLoad) => {
    ...
    return then(promise, appData.BUILD.taskQueue ? () => writeTask(update) : update);
};
...
const addHydratedFlag = (elm) => (appData.BUILD.hydratedClass ? elm.classList.add('hydrated') : appData.BUILD.hydratedAttribute ? elm.setAttribute('hydrated', '') : undefined);
...
const proxyComponent = (Cstr, cmpMeta, flags) => {
    if (appData.BUILD.member && cmpMeta.$members$) {
        ...
        members.map(([memberName, [memberFlags]]) => {
            if ((appData.BUILD.prop || appData.BUILD.state) && (memberFlags & 31 /* Prop */ || ((!appData.BUILD.lazyLoad || flags & 2 /* proxyState */) && memberFlags & 32 /* State */))) {
                // proxyComponent - prop
    ...
    if ((plt.$flags$ & 1 /* isTmpDisconnected */) === 0) {
        ...
        if (!(hostRef.$flags$ & 1 /* hasConnected */)) {
            ...
            if (appData.BUILD.slotRelocation && !hostId) {
                ...
                if (appData.BUILD.hydrateServerSide || ((appData.BUILD.slot || appData.BUILD.shadowDom) && cmpMeta.$flags$ & (4 /* hasSlotRelocation */ | 8 /* needsShadowDomShim */))) {
            ...
            if (appData.BUILD.asyncLoading) {
                ...
                while ((ancestorComponent = ancestorComponent.parentNode || ancestorComponent.host)) {
                    ...
                    if ((appData.BUILD.hydrateClientSide && ancestorComponent.nodeType === 1 /* ElementNode */ && ancestorComponent.hasAttribute('s-id')) || ancestorComponent['s-p']) {
...
const setContentReference = (elm) => {
    ...
    // create a node to represent where the original
    // content was first placed, which is useful later on
    const contentRefElm = (elm['s-cr'] = doc.createComment(appData.BUILD.isDebug ? `content-ref (host=${elm.localName})` : ''));
